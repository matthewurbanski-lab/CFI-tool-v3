...
function updateLine(solutionId, idx, patch){
  state.flightPlans[solutionId][idx] = { ...state.flightPlans[solutionId][idx], ...patch };
  saveState();
  refreshSummaryAndPreview();
}
...
function applyAutoQuantitiesFromMeasurements(){
  ...
  function setQty(solutionId, keyword, unit, value){
    ...
    if(!fp) return;
    const li = fp.find(x => (x.item||"").toLowerCase().includes(keyword) && (x.unit||"")===unit);
    if(!li) return;
    li.qty = value;
  }
...
function buildSummaryText(){
  ...
  state.suggestedSolutionIds.forEach(id=>{
    ...
    if(!fp.length){ lines.push("  (no line items)"); lines.push(""); return; }
    fp.forEach(li=>{
